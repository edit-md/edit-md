export async function fetchConnectedAccount(connectedAccount: {
	remoteId: string;
	provider: string;
}) {
...
export async function fetchConnectedAccounts(
	connectedAccounts: { remoteId: string; provider: string }[]
): Promise<{ remoteId: string; provider: string; user?: any }[]> {
